...
if __name__ == "__main__":
    ...
    state_dict = torch.load('seg.pth')
    seg.load_state_dict(state_dict)
    dataset = Dataseg()
    for image,mask in dataset:
        print(image.shape,mask.shape)
        output = seg(image.unsqueeze(0))
        plt.subplot(141)
        ...
        plt.subplot(143)
        output = torch.max(output,dim=1).indices.permute(1,2,0).detach().cpu().numpy()
        plt.imshow(output)
        plt.subplot(144)
        print(output.astype('bool')[:,:,0].shape)
        print(image.shape)
        image[output.astype('bool')[:,:,0]] = [255, 255, 255]
        plt.imshow(image / 255.)
        # plt.show()
        plt.figure()
        output = (output*255).repeat(3,-1)
        img = Image.fromarray(np.uint8(image_ori))
        mask = Image.fromarray(np.uint8(output))
        img = Image.blend(img,mask,0.7)
        plt.imshow(img)
        plt.show()
