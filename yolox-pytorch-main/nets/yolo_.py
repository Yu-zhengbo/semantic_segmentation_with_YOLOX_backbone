...
def make_five_conv_(filters_list, in_filters):
    m = nn.Sequential(
        conv2d(in_filters, filters_list[0], 1),
        conv2d(filters_list[0], filters_list[1], 3,2),
        conv2d(filters_list[1], filters_list[0], 1),
        conv2d(filters_list[0], filters_list[1], 3),
        conv2d(filters_list[1], filters_list[0], 1),
    )
...
class YOLOPAFPN(nn.Module):
    def __init__(self, depth=1.0, width=1.0, in_features=("dark3", "dark4", "dark5"), in_channels=[256, 512, 1024], depthwise=False, act="silu"):
        ...
        self.make_five_conv2 = make_five_conv_([256, 256 * 2], 128)
        self.make_five_conv3 = make_five_conv_([512, 512 * 2], 256)
        '''
    ...
    def forward(self, input):
        ...
        [x2, x1, x0] = features
        '''
        x0 = self.upsample_4(x0)
        x1 = self.upsample_2(x1)
        x2 = torch.cat([x0,x1,x2],axis=1)
        x2 = self.make_five_conv1(x2)
        x1 = self.make_five_conv2(x2)
        x0 = self.make_five_conv3(x1)
        '''
